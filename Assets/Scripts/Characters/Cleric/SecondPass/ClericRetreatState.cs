...
public class ClericRetreatState : StateBehaviour
{
    ...
    public void RunToFriend()
    {
        if (targettedFriend.Value != null)
        {
            agent.destination = targettedFriend.Value.transform.position - (targettedFriend.Value.transform.forward * 4);
